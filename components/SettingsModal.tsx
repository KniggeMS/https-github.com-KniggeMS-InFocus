      ...
      <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[95vh] animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSave} className="p-4 sm:p-6 space-y-6 overflow-y-auto custom-scrollbar">
            ...
            <div className="flex flex-wrap gap-3 pt-2">
                <button type="button" onClick={() => setShowSecrets(!showSecrets)} className="p-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl transition-colors border border-slate-700">
                    {showSecrets ? <EyeOff size={20}/> : <Eye size={20}/>}
                </button>
                <div className="flex gap-2 w-full sm:w-auto ml-auto">
                    <button type="button" onClick={onClose} className="flex-1 sm:flex-none px-4 py-3 text-slate-300 hover:text-white rounded-xl transition-colors">
                        {t('cancel')}
                    </button>
                    <button type="submit" className="flex-1 sm:flex-none px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl shadow-lg transition-all">
                        {t('save_changes')}
                    </button>
                </div>
            </div>
