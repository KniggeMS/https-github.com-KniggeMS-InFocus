...
export const DetailView: React.FC<DetailViewProps> = ({ 
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#0B0E14]/90 backdrop-blur-sm animate-in fade-in duration-300 p-4 md:p-8">
            <div className="bg-[#0B0E14] w-full max-w-6xl h-full md:h-auto md:max-h-[85vh] rounded-[2rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,1)] border border-white/5 flex flex-col md:flex-row overflow-hidden relative">
                ...
                <div className="w-full md:w-[380px] shrink-0 bg-black relative flex items-center justify-center overflow-hidden">
                    {!showTrailer && backgroundTrailerUrl && (
                        <div className="absolute inset-0 opacity-30 grayscale-[0.3]">
                ...
                <div className="flex-grow flex flex-col overflow-hidden bg-gradient-to-br from-[#0B0E14] to-[#11141d]">
                    <div className="p-8 md:p-12 pb-0">
                        <div className="flex flex-wrap gap-2 mb-4">
                            <span className="px-2 py-0.5 rounded bg-cyan-500/20 text-cyan-400 text-[10px] font-black uppercase tracking-widest border border-cyan-500/20">{displayItem.type}</span>
                            {displayItem.status && <span className="px-2 py-0.5 rounded bg-orange-500/20 text-orange-400 text-[10px] font-black uppercase tracking-widest border border-orange-500/20">{t(displayItem.status.toLowerCase())}</span>}
                        </div>
                        <h1 className="text-4xl md:text-6xl font-black text-white leading-tight mb-6">{displayItem.title}</h1>
                        <div className="flex flex-wrap items-center gap-6 text-slate-400 font-bold text-sm mb-8">
                            ...
                            {displayItem.runtime && <span className="flex items-center gap-2"><Clock size={16} /> {Math.floor(displayItem.runtime/60)}h {displayItem.runtime%60}m</span>}
                            <div className="flex items-center gap-3">
                                <div className="bg-[#0d253f] px-2 py-0.5 rounded border border-[#01b4e4]/30 flex items-center gap-1.5">
                                    <span className="text-[9px] font-black text-[#01b4e4]">TMDB</span>
                        ...
                        <div className="flex gap-4 mb-8">
                            {isExisting ? (
                                <button onClick={() => onToggleFavorite && (initialItem as MediaItem).id && onToggleFavorite((initialItem as MediaItem).id)} className={`px-6 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 border ${isFav ? 'bg-white/10 text-white border-white/20' : 'bg-white/5 border-white/10 text-slate-300 hover:text-white'}`}>
                                    <Heart size={18} fill={isFav ? "currentColor" : "none"} className={isFav ? "text-red-500" : ""} /> {isFav ? 'Favorisiert' : 'Favorisieren'}
                                </button>
                            ) : (
                                <button onClick={() => onAdd && onAdd(initialItem as SearchResult, WatchStatus.TO_WATCH, false)} className="px-6 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl shadow-lg transition-all flex items-center gap-2">
                                    <Clock size={18} /> {t('watchlist')}
                                </button>
                            )}
                            <button onClick={handleShare} className="px-6 py-2.5 bg-white/5 hover:bg-white/10 text-white rounded-xl border border-white/10 transition-all flex items-center gap-2 font-bold">
                        ...
                        <div className="max-w-3xl space-y-10">
                            {activeTab === 'overview' && (
                                <>
                                    ...
                                    <div className="bg-[#1a1525]/80 backdrop-blur-md p-8 rounded-3xl border border-purple-500/20 relative overflow-hidden">
                                         ...
                                         <p className="text-purple-100 italic text-lg leading-relaxed">{loadingAi ? "Analysiere..." : (aiAnalysis || "Analyse bereit.")}</p>
                                    </div>
                                    {isExisting && (
                                        <div className="space-y-4 pt-6 border-t border-white/5">
                                            <div className="flex justify-between items-center">
                                                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">DEINE REZENSION</h3>
                                                <span className="text-[9px] bg-green-950/30 text-green-400 px-2 py-0.5 rounded-full border border-green-500/20 font-black uppercase">Öffentlich</span>
                                            </div>
                                            <textarea 
                                                value={notes} onChange={e => setNotes(e.target.value)} onBlur={handleSaveNotes}
                                                placeholder="Teile deine Meinung..."
                                                className="w-full bg-black/40 border border-white/10 rounded-2xl p-6 text-slate-200 text-lg focus:border-cyan-500 transition-all min-h-[140px] resize-none"
                                            />
                                        </div>
                                    )}
                                </>
                                ...
                                <div className="flex flex-wrap gap-8 py-4">
                                    {displayItem.providers?.length ? displayItem.providers.map(p => (
                                        <div key={p.providerId} className="flex flex-col items-center gap-3 group">
                                            <div className="w-16 h-16 rounded-2xl overflow-hidden shadow-xl border border-white/10 transition-transform bg-slate-800">
