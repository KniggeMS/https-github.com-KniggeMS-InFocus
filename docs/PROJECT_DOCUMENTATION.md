...
**Version:** 2.0.0 ("The Perfect State")

---

## 1. Release Notes (v2.0.0)
...
- **Translation Keys:** Korrektur der Status-Label von Uppercase (TO_WATCH) auf Lowercase-Mapping (to_watch -> "Geplant"), um konsistente deutsche Badges zu garantieren.
...
1. **System-Keys:** Werden über Vercel Environment Variables (`VITE_TMDB_API_KEY`, etc.) injiziert.
2. **User-Overrides:** Nutzer können in den Einstellungen eigene Keys hinterlegen, die im LocalStorage gespeichert werden.

**Checkliste für Updates:**
- [ ] Immer `selectedItem` auf Null-Werte prüfen.
- [ ] Neue Sprachschlüssel müssen in `LanguageContext.tsx` für DE und EN hinterlegt werden.
- [ ] CSS-Änderungen sollten die `glass-panel` Utility nutzen.

---
*Dokumentation gesichert am: ${new Date().toLocaleDateString('de-DE')}*
