                ...
                <div className="relative" ref={profileMenuRef}>
                    ...
                    {isProfileMenuOpen && (
                        <div className="absolute right-0 mt-2 w-64 bg-[#151a23] border border-white/10 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.6)] py-2 z-[110] animate-in fade-in zoom-in-95 duration-200">
                            ...
                            <button onClick={() => { navigate('/guide'); setIsProfileMenuOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:bg-white/5 flex items-center gap-3 transition-colors"><BookOpen size={16} className="text-slate-500" /> {t('handbuch') || 'Handbuch'}</button>
                            {isAdmin && (
                                <button onClick={() => { navigate('/users'); setIsProfileMenuOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:bg-white/5 flex items-center gap-3 transition-colors"><Shield size={16} className="text-cyan-500" /> {t('user_management') || 'Benutzerverwaltung'}</button>
                            )}
