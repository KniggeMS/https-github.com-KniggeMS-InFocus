...
import { LogOut, Search, Settings, User as UserIcon, List, Heart, Clapperboard, Plus, X, ChevronDown, UserPlus } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0E14] text-slate-200 pb-20 md:pb-0 font-sans relative overflow-hidden">
                ...
                <div className="relative" ref={profileMenuRef}>
                    ...
                    {isProfileMenuOpen && (
                        <div className="absolute right-0 mt-2 w-56 glass-panel rounded-xl shadow-2xl py-1.5 z-50 animate-in fade-in zoom-in-95 duration-200">
                            <div className="px-4 py-3 border-b border-white/5 mb-1"><p className="text-sm font-bold text-white truncate">{user.username}</p></div>
        ...
        <div className="flex max-w-[1600px] mx-auto relative z-10">
            <aside className="hidden md:flex w-64 flex-col fixed left-0 top-16 bottom-0 border-r border-white/5 bg-[#0B0E14]/50 backdrop-blur-sm overflow-y-auto">
                <div className="p-4">
                    <button onClick={() => setIsSearchOpen(true)} className="w-full flex items-center gap-2 bg-cyan-600 hover:bg-cyan-500 text-white px-4 py-3 rounded-xl font-bold transition-all mb-6"><Plus size={20} /> {t('add_button')}</button>
                    <div className="mb-6">
                        <h3 className="px-3 text-xs font-bold text-slate-500 uppercase mb-2">{t('my_lists')}</h3>
                        <div className="space-y-1">
                            {myLists.map(l => (
                                <div key={l.id} className="group flex items-center justify-between pr-2 rounded-lg hover:bg-white/5">
            ...
            <main className="flex-grow p-4 md:p-8 min-h-[calc(100vh-64px)] md:pl-64">
        ...
        {/* CRITICAL FIX: Conditionally render DetailView ONLY if selectedItem exists */}
        {selectedItem && (
            <DetailView 
                item={selectedItem} 
                isExisting={true} 
